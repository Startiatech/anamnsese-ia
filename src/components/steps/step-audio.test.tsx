...
describe('StepAudio — multi-segmento', () => {
  async function startRecordingNth(mockStream: MediaStream, expectedCount: number) {
    vi.stubGlobal(
      'navigator',
      { mediaDevices: { getUserMedia: vi.fn().mockResolvedValue(mockStream) } },
    )
    ...
    await waitFor(() => expect(MockMediaRecorder.instances).toHaveLength(expectedCount))
  }

  it('concatena múltiplos segmentos gravados em um único upload', async () => {
    const fetchSpy = vi.spyOn(global, 'fetch').mockResolvedValue(
      makeStreamResponse('texto transcrito'),
    )
    ...
    // Primeiro segmento: grava e finaliza
    await startRecordingNth(mockStream, 1)
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /finalizar/i }))
    })

    // Retorna ao estado idle clicando em Regravar
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /regravar/i }))
    })

    // Segundo segmento: grava e finaliza
    await startRecordingNth(mockStream, 2)
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /finalizar/i }))
    })
    ...
    const sentForm = (init as RequestInit).body as FormData
    const audio = sentForm.get('audio') as File
    expect(audio).toBeInstanceOf(Blob)
    // Each segment mock fires ondataavailable with Blob(['audio-data']) = 10 bytes.
    // With accumulation both segments are combined: size must be > one segment alone.
    const singleSegmentSize = new Blob(['audio-data'], { type: 'audio/webm' }).size
    expect(audio.size).toBeGreaterThan(singleSegmentSize)
  })
})
